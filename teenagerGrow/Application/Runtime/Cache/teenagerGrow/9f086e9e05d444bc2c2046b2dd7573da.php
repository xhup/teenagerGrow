...
<head>
    ...
    <title>学龄前体检记录列表</title>
</head>
<body>
<table id="schoolBeforeRecordTable" class="detailTable" align="center" border="0" width="90%" cellspacing="0" cellpadding="0">
    <thead>
    <tr>
        <th>就诊号</th>
        <th>年龄</th>
...
<script>
    ...
    $(document).ready(function() {
        var dataSource="<?php echo U('LookChild/allCheckRecordList','dateForm=PreschoolAge');?>";
        $('#schoolBeforeRecordTable').DataTable( {
            "ajax": dataSource,
            "lengthChange":false,
            "searching": false,
            language: {
...
<script>
    $(function(){
        $("#schoolBeforeRecordTable tbody").on("click","button",function(){
            var value =  $(this).parents("tr").children("td:eq(2)").text();//获取对应的体检日期
            //将点击的体检记录的体检日期存到后台并加载相应的具体体检记录
            var target="saveCheckDate";
            var url="schoolBeforeCheck.html";
