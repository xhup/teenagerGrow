...
<body>
<div>
    <table id="childFamilySickTable" class="detailTable" align="center" border="0" width="90%" cellspacing="0" cellpadding="0">
        <thead>
        <tr>
            <th>成员姓名</th>
            <th>性别</th>
...
<script>
    ...
    $(document).ready(function() {
        var dataSource="<?php echo U('LookChild/allFamilySickList');?>";
        $('#childFamilySickTable').DataTable( {
            "ajax": dataSource,
            "lengthChange":false,
            "searching": false,
            language: {
...
<script>
    $(function(){
        $("#childFamilySickTable tbody").on("click","button",function(){
            var value =  $(this).parents("tr").children("td:eq(0)").text();//获取对应的家族成员姓名
            //将点击的体检记录的体检日期存到后台并加载相应的具体体检记录
            var target="saveMemberName";
            $.post(target,{"memberName":value},function(data){
                var url="childFamilySick.html";
                if(data){
