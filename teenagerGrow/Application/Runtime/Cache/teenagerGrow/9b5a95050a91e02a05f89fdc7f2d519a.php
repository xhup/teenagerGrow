...
<head>
    ...
    <title>婴儿访视记录列表</title>
</head>
<body>
<div>
<table id="babyVisitRecordTable" class="detailTable" align="center" border="0" width="90%" cellspacing="0" cellpadding="0">
    <thead>
    <tr>
        <th>就诊号</th>
        <th>访视日期</th>
...
<script>
    ...
    $(document).ready(function() {
        var dataSource="<?php echo U('LookChild/allVisitRecordList');?>";
        $('#babyVisitRecordTable').DataTable( {
            "ajax": dataSource,
            "lengthChange":false,
            "searching": false,
            language: {
...
<script>
    $(function(){
        $("#babyVisitRecordTable tbody").on("click","button",function(){
            var value =  $(this).parents("tr").children("td:eq(1)").text();//获取对应的访视日期
            //将点击的体检记录的体检日期存到后台并加载相应的具体体检记录
            var target="saveCheckDate";
            $.post(target,{"checkDate":value},function(data){
                var url="babyVisit.html";
                if(data){
